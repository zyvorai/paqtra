...
import { HeartPulse, Loader2, CheckCircle, XCircle, AlertTriangle, Server, Box, CircleDot } from 'lucide-react';
    ...
    <div>
      ...
      {!loading && !health && (
        <div className="text-center py-12 text-slate-400">
          <HeartPulse className="w-12 h-12 mx-auto mb-3 text-rose-400" />
          <div className="font-medium text-white">No health data available</div>
          <div className="text-sm">Cluster health information is not yet available.</div>
        </div>
      )}
      ...
      {health && (
        <>
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2"><div className="w-1 h-5 bg-gradient-to-b from-green-400 to-emerald-500 rounded-full" /><h2 className="text-lg font-semibold text-white">Component Status</h2></div>
            {(health.components ?? []).length > 0 && <ExportButton data={(health.components ?? []) as unknown as Record<string, unknown>[]} filename="cluster-health-components" />}
          </div>
