...
```rust
use cilium_vision::modules::replay::*;
...
```bash
# Record traffic
cilium-vision record start --name "baseline" --duration 5m

# List recordings
cilium-vision record list

# Replay
cilium-vision replay rec-123 --cluster staging

# Compare
cilium-vision replay compare rec-123 --against rec-456

# Filter replay
cilium-vision replay rec-123 --namespace prod --port 5432

# Export recording
cilium-vision record export rec-123 --output baseline.json.gz

# Import recording
cilium-vision record import baseline.json.gz

# Cleanup old recordings
cilium-vision record cleanup --older-than 7d
```
